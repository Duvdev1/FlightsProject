    ...
    def get_instance(cls):
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                cls._instance.formatter = logging.Formatter(f''
                                                            f'%(asctime)s:%(module)s:%(process)d:%(thread)d:'
                                                            f'%(levelname)s:%(message)s')
                cls._instance.file_handler = logging.FileHandler(f'{Logger.LOG_FILE_NAME_PREFIX}.'
                                                                 f'{Logger.LOG_FILE_NAME_EXT}')
